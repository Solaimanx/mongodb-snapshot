...
import {
  ...
  SourceConnectorBaseOptions,
  TargetConnectorBaseOptions,
} from "../Connector";
import {
  MongoDBConnection,
...
const schema = joi.object(<Schema<MongoDBConnectorOptions>>{
  connection: joi
    .object(<Schema<MongoDBConnection>>{
    ...
    .required(),
  assource: joi
    .object(<Schema<AsSourceMongoDBOptions>>{
      ...SOURCE_CONNECTOR_BASE_OPTIONS_SCHEMA,
    })
    .required(),
  astarget: joi
    .object(<Schema<AsTargetMongoDBOptions>>{
      ...TARGET_CONNECTOR_BASE_OPTIONS_SCHEMA,
...
export class MongoDBDuplexConnector
  extends Validatable
  implements SourceConnector, TargetConnector
{
  ...
  // as source
  chunk$({ name: collection_name }: CollectionMetadata) {
    return defer(async () => {
      ...
      if (
        !this.collections.find(
          (collection) => collection.collectionName === collection_name
        )
      ) {
        return EMPTY;
      }
      ...
      let chunkCursor = collection.find<Buffer>(
        {},
        {
  ...
  async exists() {
    if (!this.db || !this.client) {
      throw new Error(
        "Need to connect to the data source before using this method."
      );
    }
  ...
  writeCollectionData(
    ...
    const documents$ = convertAsyncGeneratorToObservable(
      getDocumentsGenerator(chunk$)
    ).pipe(
      filter(({ obj: document }) => {
    ...
    return documents$.pipe(
      ...
      mergeMap(async (documents: CollectionDocument[]) => {
        if (documents.length > 0) {
          await this.writeCollectionDocuments(
            collection_name,
  ...
  async writeCollectionDocuments(
    ...
    const replaceFilter = Object.entries(upsert || {}).reduce(
      (
        docFilterFn: DocumentFilterFunction | undefined,
        [collectionSelector, collectionDocumentFilters]
      ) => {
        if (hasRegexMatch(collectionSelector, collectionName)) {
          collectionDocumentFilters = !isArray(collectionDocumentFilters)
            ? [collectionDocumentFilters]
            : collectionDocumentFilters;

          const currCollectionDocumentFilterFn =
            collectionDocumentFilters.reduce<
              DocumentFilterFunction | undefined
            >((collectionDocumentFilterFn, filter) => {
              let documentFilterFn: DocumentFilterFunction | undefined =
                undefined;
          ...
          if (currCollectionDocumentFilterFn) {
            return (document) => {
              if (docFilterFn) {
                return <FilterQuery<any>>{
                  $or: [
                    ...
                    (currCollectionDocumentFilterFn as DocumentFilterFunction)(
                      document
                    ),
                  ],
                };
              ...
              return (currCollectionDocumentFilterFn as DocumentFilterFunction)(
                document
              );
            };
          }
    ...
    return write(documents).catch(async function () {
      ...
      documents.forEach(({ obj: document }) =>
        filterInvalidKeys(document, (key) => key && key.includes("."))
      );
        ...
        all_collections.map(async (collection) => {
          try {
            ...
            console.warn(
              ...
              }", as we couldnt receive details due to an error: ${
                (e as any).message
              }`
            );
...
function filterInvalidKeys(
  obj: { [key: string]: any },
  filterKeyFn: (key: string) => any
) {
...
async function* getDocumentsGenerator(
  chunk$: Observable<Buffer>
): AsyncGenerator<CollectionDocument> {
